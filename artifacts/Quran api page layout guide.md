# Quran.com API - Page Layout Guide

Official documentation for the Pages Lookup API - understand and implement Mushaf pagination.
...
3. [Understanding Mushafs](#understanding-mushafs)
4. [Pages Lookup API](#pages-lookup-api)
5. [Verses API Integration](#verses-api-integration)
6. [Implementation Patterns](#implementation-patterns)
7. [View Modes](#view-modes)
8. [Common Use Cases](#common-use-cases)
9. [Font Scale & Layout](#font-scale--layout)
...
### Pattern 3: Framework-Specific Implementations
...
- `PagesLookup`: Model for API response with lookupRange, pages map, and totalPage
- `PageInfo`: Model for individual page with from and to verse keys
- `QuranApiService`: API client with `getPagesLookup()` and `getPageVerses()` methods
...
- `PageInfo`: Dataclass with from_verse, to_verse, first_verse_key, last_verse_key
- `PagesLookup`: Dataclass with lookup_range, pages dict, and total_page
- `QuranAPI`: Client class with `get_pages_lookup()` and `get_page_verses()` methods
...
**Word Layout Metadata:** Each word includes:

- `page_number` (1-604)
- `line_number` (1-15)
- `position` (order within verse)
...
async function getCachedPagesLookup(chapterNumber, mushafId) {
  const cacheKey = `${chapterNumber}-${mushafId}`;
...
```javascript
async function prefetchAdjacentPages(currentPage, mushafId, pagesLookup) {
  const pages = Object.keys(pagesLookup.pages).map(Number);
  ...
  await Promise.all(
    pagesToFetch.map(page => fetchAndCachePageVerses(page, mushafId))
  );
}
```
...
```javascript
async function switchMushaf(currentVerseKey, newMushafId) {
  // Look up which page the verse is on in the new Mushaf
  const lookup = await fetchPagesLookup(currentVerseKey, newMushafId);
  const newPage = Object.keys(lookup.pages)[0];
  
  return {
    page: newPage,
    pageInfo: lookup.pages[newPage],
    mushafId: newMushafId
  };
}
```
...
const MAX_PAGES = {
  ...
  6: 610,  // IndoPak 15-line
  7: 548,  // IndoPak 16-line
  11: 604, 19: 604
};

function isValidPage(pageNumber, mushafId) {
  return pageNumber >= 1 && pageNumber <= MAX_PAGES[mushafId];
}
```
...
```javascript
// ✅ Correct
fetch(`${API_BASE}/pages/lookup?chapter_number=2&mushaf=1`)

// ❌ Wrong - invalid mushaf ID
fetch(`${API_BASE}/pages/lookup?chapter_number=2&mushaf=99`)
```
...
```javascript
function isVerseSpanningPages(verse) {
...
const totalVerses = parseVerseKey(data.lookupRange.to).verse - 
                    parseVerseKey(data.lookupRange.from).verse + 1;
```
...
- **Full API Documentation:** <https://api-docs.quran.foundation>
- **Pages Lookup API Reference:** <https://api-docs.quran.foundation/docs/tutorials/fonts/page-layout>
- **Verses API Reference:** <https://api-docs.quran.foundation/docs/category/content-apis>
- **Developer Support:** <https://discord.gg/SpEeJ5bWEQ>

---

**Official documentation from Quran.foundation**
